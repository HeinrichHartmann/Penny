...
def test_group_id_never_null(storage_with_accounts: TransactionStorage):
    ...
    stored = storage_with_accounts.list_transactions(limit=1, consolidated=False)
    assert len(stored) == 1
...
def test_consolidated_query_groups_entries(storage_with_accounts: TransactionStorage):
    ...
    raw = storage_with_accounts.list_transactions(consolidated=False)
    assert len(raw) == 2

    # Consolidated: 1 entry with net amount
    consolidated = storage_with_accounts.list_transactions(consolidated=True)
